...
<template>
  <section class="flex flex-col items-center w-full justify-center bg-no-repeat bg-cover bg-[url(assets/imgs/header-bg.png)] pt-50 pb-60 bg-gradiente">
    <h1 class="text-5xl text-[#306067]">Planes</h1>
    <h2 class=" text-center text-2xl text-[#37A0AF] mb-5 ">Elige el plan que mejor se adapte a tus necesidades.</h2>

    <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 gap-6 mt-3 items-stretch container mx-auto">
      <div v-for="plan in plans" :key="plan.id" :class="[
        ...
        <div :class="['flex', 'flex-col', 'py-6', 'px-8', 'rounded-2xl', plan.main ? 'bg-white/80' : ' ', 'w-full', 'h-full']">
          ...
          <span class="text-5xl font-extrabold text-[#37A0AF] mb-4 text-center">{{ plan.price }}</span>
          ...
          <ul class="flex flex-col gap-2 text-[#2A4043] text-start mb-6">
            <li v-for="(feature, i) in plan.features" :key="i" class="flex items-center gap-2">
              <span class="text-[#37A0AF] font-bold">✔</span> {{ feature }}
            </li>
          </ul>
